...
import io.runebox.revtools.mapper.NameType;
import io.runebox.revtools.mapper.SimilarityChecker;
import io.runebox.revtools.mapper.Util;
import io.runebox.revtools.mapper.bcremap.AsmClassRemapper;
...
public final class ClassInstance implements Matchable<ClassInstance> {
	...
	public MethodInstance[] getMemberMethods() {
		return Arrays.stream(methods).filter(m -> !m.isStatic).toArray(MethodInstance[]::new);
	}
