...
public final class ClassEnvironment implements ClassEnv {
	...
	public Collection<MethodInstance> getStaticMethodsA() {
		ArrayList<MethodInstance> list = new ArrayList<>();
		for(ClassInstance cls : getClassesA()) {
			Collections.addAll(list, cls.getStaticMethods());
		}
		return list;
	}

	public Collection<MethodInstance> getStaticMethodsB() {
		ArrayList<MethodInstance> list = new ArrayList<>();
		for(ClassInstance cls : getClassesB()) {
			Collections.addAll(list, cls.getStaticMethods());
		}
		return list;
	}
