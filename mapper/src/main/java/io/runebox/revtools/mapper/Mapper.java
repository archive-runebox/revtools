...
import io.runebox.revtools.mapper.classifier.*;
import io.runebox.revtools.mapper.config.Config;
...
public class Mapper {
	...
	public Mapper(ClassEnvironment env) {
		this.env = env;
	}
	...
	public boolean autoMatchStaticMethods(ClassifierLevel level, double absThreshold, double relThreshold, DoubleConsumer progressReceiver) {
		AtomicInteger totalUnmatched = new AtomicInteger();
		Map<MethodInstance, MethodInstance> matches = match(level, absThreshold, relThreshold,
				cls -> cls.getEnv().getStaticMethods().toArray(MethodInstance[]::new), MethodClassifier::rank, MethodClassifier.getMaxScore(level),
				progressReceiver, totalUnmatched);
	...
	public void doMatch(ClassInstance clsA, ClassInstance clsB, MemberInstance<?> memberA, MemberInstance<?> memberB, MethodVarInstance varA, MethodVarInstance varB) {
		...
		if (canMatchMembers(memberA, memberB)) {
			if (memberA instanceof MethodInstance) {
				match((MethodInstance) memberA, (MethodInstance) memberB);
			} else {
				match((FieldInstance) memberA, (FieldInstance) memberB);
			}
	...
	public void matchPerfectMembers(ClassInstance cls) {
		...
		for (Map.Entry<MethodInstance, MethodInstance> entry : matchedMethods.entrySet()) {
			...
			match(entry.getValue(), entry.getKey());
		...
		for (FieldInstance m : clsA.getFields()) {
			...
			List<RankResult<FieldInstance>> results = FieldClassifier.rank(m, clsB.getFields(), ClassifierLevel.Full, getEnv());
	...
	private void unmatchClass(ClassInstance cls) {
	...
	public void unmatchMember(MemberInstance<?> member) {
	...
	public void unmatchVar(MethodVarInstance var) {
